"""Native Kimi (Moonshot) API wrapper for HalOS."""
...
class KimiAPI:
    ...
    async def chat(self, user_message: str, source: str = "telegram",
                   use_heavy: bool = False, model: str = None, personality_override: str = None) -> str:
        """Send a message to Kimi and return the response.

        Args:
            user_message: The message from the user
            source: Source identifier for the conversation
            use_heavy: Whether to use the heavy/capable model
            model: Optional model override (uses config default if not provided)
            personality_override: Optional personality to use instead of default
        """
        ...
        model = model or self.config.kimi.model

        start = time.time()
        try:
            response = await self.client.chat.completions.create(
                model=model,
                max_tokens=self.config.kimi.max_tokens,
                messages=messages,
            )
    ...
    async def complete(self, prompt: str, system: str = None,
                       use_heavy: bool = False, model: str = None) -> str:
